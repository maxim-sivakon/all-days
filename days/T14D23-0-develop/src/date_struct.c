...
void struct_output(struct date_struct buff) {
    printf("%d %d %d %d %d %d %d %d", buff.year, buff.month, buff.day, buff.houre, buff.minute, buff.sec,
           buff.status, buff.code);
}
...
int date_struct_input(struct date_struct *date) {
    ...
    if (scanf("%d %d %d %d %d %d %d %d%c", &y, &mo, &d, &h, &min, &se, &stat, &cod, &c) == 9 &&
        (c == '\n' || c == '\0')) {
        date->year = y;
        date->month = mo;
        date->day = d;
